...
dt = args.dt
NSTEPS = 1
if model_indx in [0, 1, 2, 3, 4]:
    # Esch-Levine models
    NSTEPS = int(1000.0/dt)
    
elif model_indx in [5,6]:
    # Holstein odel
    NSTEPS = int(10000.0/dt)
    
elif model_indx in [7,8]:
    # Spin-Boson
    NSTEPS = int(10000.0/dt)
...
print(F"method_indx = {method_indx}")
if method_indx == 0:
    ...
    gfsh.load(dyn_general) # GFSH, with F-tracking - direction-informed
    dyn_general.update({"state_tracking_algo":4, "do_phase_correction":0 })  # standard state tracking
    dyn_general.update({"rep_tdse":1, "electronic_integrator":3, "do_nac_phase_correction":2 })   # 1-point, Hvib integration, with exp_


print(F"dyn_general = {dyn_general}")
